...
def ai_analyze(news, period_str, risk_data, today_term):
    ...
    ⚠️【最高指令一】：請使用標準的「-」或數字「1. 2. 3.」作為條列項目開頭，句首一律不可以加星號。但必須保留【重大事件】中的重要性評分（如 ★★★★★）。
    ⚠️【最高指令二】：嚴禁輸出任何問候語（如「好的長官」、「我是分析官」等廢話）。你的回答第一行必須直接是「🎯 【一分鐘戰略速讀】」。
    ⚠️【最高指令三】：如果今日日期是每個月的 1 號（不管有無交易），請務必在報告中針對台灣宏觀景氣循環進行「長線投資觀察」的戰略補充。

    🎯 【一分鐘戰略速讀】
    請撰寫約 150~200 字的精華摘要（設計為剛好適合語音播報 45~60 秒的長度）。請用 3 到 4 個結構完整的列點，深度解析全局多空定調、資金板塊轉移邏輯，以及潛在風險警戒。
    ...
    🔥 【市場情緒與壓力測試】
    必須「明確引述」提供的 VIX、匯率，以此作為籌碼與情緒壓力的佐證！
