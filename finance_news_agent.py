...
def ai_analyze(news, period_str):
    ...
    if period_str == "盤前":
        ...
        # 💡 [監視器]：在 GitHub 後台印出真實抓到的數據，方便您隨時查核爬蟲狀態
        print("\n=== 🕵️‍♂️ 系統抓到的盤前真實數據 ===")
    ...
    ⛔ 【絕對禁止事項】：嚴禁寫出「素材未提及 VIX」等廢話。
    ✅ 【強制執行事項】：您必須在報告中「明確引述」當前的 VIX 數值與融資餘額數據，並以此作為您判斷籌碼與恐慌壓力的佐證！
        """
        pressure_test_format = "📊【市場情緒與壓力測試】 (⚠️請務必在此段落『寫出具體的 VIX 與融資數值』，再綜合評估避險需求與籌碼壓力)"
    
    else: # 🚨 這裡是您剛剛漏掉的 else 區塊！這是「盤後」和「週末」的邏輯！
        market_data_section = """
