...
def ai_analyze(news):
    ...
    # 你原本的格式 + 小幅強化：要求更像「快報」且有結論
    prompt = f"""
你是總體經濟分析師與台股策略研究員。
請對以下新聞做：
1) 重要性排序（列出 3-6 則最重要）
2) 市場情緒：偏風險偏好 / 風險趨避 / 中性（簡述原因）
3) 台股影響：利多 / 中性 / 利空（若可能，點名產業：半導體、金融、航運、AI伺服器、內需等）
4) 投資觀察：給 3-5 點可操作觀察（避免給出保證獲利語氣）
...
if __name__ == "__main__":
    print("🚀 Daily finance agent start")
    run_daily()
    print("✅ done")